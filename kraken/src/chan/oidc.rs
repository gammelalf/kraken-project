//! The openid connect client used to authenticate users
...
/// The state of an ongoing oidc login stored in the user's session
...
/// The state of an ongoing oidc login returned by the oidc provider
#[derive(Debug, Serialize, Deserialize)]
#[allow(missing_docs)]
...
impl OpenIdConnect {
    /// Starts an openid connect login request
    ///
    /// The user should be redirected to the returned request
    /// while storing the returned state in his session.
    pub fn begin_login(&self) -> (Url, OidcSessionState) {
    ...
    /// Finishes an openid connect login request
    pub async fn finish_login(
        ...
        let token_response = self
            ...
            .map_err(|error| match error {
                RequestTokenError::ServerResponse(_) => ApiError::Unauthenticated,
                _ => {
        ...
        let claims = id_token
            ...
            .map_err(|_error| ApiError::Unauthenticated)?;
        ...
        if let Some(expected_access_token_hash) = claims.access_token_hash() {
            let actual_access_token_hash = AccessTokenHash::from_token(
                ...
                id_token.signing_alg().map_err(|error| {
                    error!("Failed to retrieve signing algorithm: {error}");
                    ApiError::InternalServerError
                })?,
                id_token.signing_key(&id_token_verifier).map_err(|error| {
                    error!("Failed to retrieve signing key: {error}");
                    ApiError::InternalServerError
                })?,
            )
            .map_err(|error| {
                error!("Failed to recreate access token signature: {error}");
                ApiError::InternalServerError
            })?;
...
impl OpenIdConnect {
    /// Uses the `OIDC_...` env variables to discover the open id connect provides setup
    pub async fn discover() -> Result<Self, DiscoveryError<HttpClientError<reqwest::Error>>> {
        #[allow(clippy::unwrap_used, reason = "It's just a timeout")]
